...
const workDir = path.join(__dirname, 'vivienne_linear_prod');
const ttsDir = path.join(workDir, 'tts');

const userProfile = process.env.USERPROFILE || 'C:\\Users\\fenou';
const outDownloads = path.join(userProfile, 'Downloads', 'MICROSCOPE_JAUNE_VIVIENNE_PRO.mp4');
const outDesktop = path.join(userProfile, 'Desktop', 'MICROSCOPE_JAUNE_VIVIENNE_PRO.mp4');
const outPublic = path.join(__dirname, '..', 'public', 'videos', 'microscope-jaune-vivienne-pro.mp4');
...
// 4 plans strictement linéaires et logiques
const segments = [
  // 1. LE HOOK UNIQUE (Brocoli - 0s à 3s)
  {
    src: v3Brocoli,
    ss: 0.0,
    srcLen: 3.2,
  ...
  // 2. LE DÉBALLAGE FLUIDE SUR FOND BLANC
  {
    src: v2Deballage,
    ss: 0.0,
    srcLen: 4.8,
  ...
  // 3. LA DÉMONSTRATION DU ZOOM SUR NOTICE
  {
    src: v2Deballage,
    ss: 5.5,
    srcLen: 5.8,
...
async function run() {
  console.log('🎬 GÉNÉRATION AVEC LA VOIX VIVIENNE & MASTERING AUDIO PUISSANT...');
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const audioFile = path.join(ttsDir, `voice_vivienne_${i}.mp3`);
  ...
  // Mixage : Traitement de présence voix (volume=1.65) + compresseur vocal + Musique très subtile (volume=0.07)
  console.log('4. Mixage audio broadcast (Voix forte & présente + musique fond)...');
  ...
  // Rendu Final avec sous-titres fixes en bas
  console.log('6. Rendu final avec sous-titres...');
  ...
  console.log(`\n🎉 SUCCÈS : NOUVELLE VIDÉO VIVIENNE TERMINÉE ET DISPONIBLE !`);
