...
function computeStats(sessions: AnalyticsSession[]): AnalyticsStats {
  ...
  // Filtrer les sessions avec une vraie présence (> 2s) pour calculer la moyenne de temps
  const validDurationSessions = sessions.filter((s) => (s.durationSeconds || 0) > 2);

  if (validDurationSessions.length === 0) {
    return { totalViews, totalClicks, ctr, avgTimeSpentSeconds: 0, formattedAvgTime: "—" };
  }

  const totalTime = validDurationSessions.reduce(
    (acc, s) => acc + (s.durationSeconds || 0),
    0
  );
  const avgTimeSpentSeconds = Math.round(totalTime / validDurationSessions.length);
