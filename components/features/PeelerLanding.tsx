    ...
    <div className="min-h-screen bg-[#FDFBF7] text-slate-900 selection:bg-blue-600/20 selection:text-blue-900">
      ...
      <section id="commander" className="py-14 px-4 md:px-8 max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center max-w-xl mx-auto">
          ...
          <h2 className="font-display font-extrabold text-2xl sm:text-3xl text-slate-900 mt-2">
            Choisissez votre Pack & Recevez votre Machine
          </h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Paiement 100% à la livraison après vérification du colis.
          </p>
        </div>
            ...
            return (
              <div
                ...
                className={`rounded-2xl p-5 border-2 transition-all duration-150 cursor-pointer relative flex flex-col justify-between ${
                  isSel
                    ? "bg-blue-50/70 border-[#0047AB] shadow-md scale-[1.02]"
                    : "bg-white border-slate-200/80 hover:border-slate-300"
                }`}
              >
                {b.badge && (
                  <span className="absolute -top-3 right-4 bg-[#0047AB] text-white text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow-xs">
                    {b.badge}
                  </span>
                )}
          ...
          {submitted ? (
            <div className="text-center py-6 sm:py-8 space-y-5 animate-fadeIn max-w-lg mx-auto">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-slate-600 mt-2 leading-relaxed">
                  Votre commande du <strong>{selected.name}</strong> ({fmt(selected.price)}) a été transmise à notre service logistique. Nous vous contacterons au <strong>{phone}</strong> pour planifier votre livraison.
                </p>
              </div>
              ...
              {/* 3 ÉTAPES CLAIRES */}
              <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 sm:p-5 text-left space-y-2.5 text-xs text-slate-700">
                <div className="font-bold text-[11px] uppercase tracking-wider text-[#0047AB] mb-1">
                  Prochaines étapes :
                </div>
                <div className="flex gap-2.5 items-start">
                  <span className="w-5 h-5 rounded-full bg-blue-200 text-[#0047AB] font-bold flex items-center justify-center shrink-0 text-[10px]">1</span>
                  <div>
                    <span className="font-bold text-slate-900">Appel de confirmation :</span>
                    <p className="text-slate-500 text-[11px]">Notre équipe vous appelle pour valider l&apos;heure et l&apos;adresse exacte.</p>
                  </div>
                </div>
                <div className="flex gap-2.5 items-start">
                  <span className="w-5 h-5 rounded-full bg-blue-200 text-[#0047AB] font-bold flex items-center justify-center shrink-0 text-[10px]">2</span>
                  <div>
                    <span className="font-bold text-slate-900">Expédition express 24h–48h :</span>
                    <p className="text-slate-500 text-[11px]">Le livreur vous remet votre colis scellé à {city} et environs.</p>
                  </div>
                </div>
                <div className="flex gap-2.5 items-start">
                  <span className="w-5 h-5 rounded-full bg-blue-200 text-[#0047AB] font-bold flex items-center justify-center shrink-0 text-[10px]">3</span>
                  <div>
                    <span className="font-bold text-slate-900">Inspection & Paiement à la réception :</span>
                    <p className="text-slate-500 text-[11px]">Vous ouvrez, inspectez l&apos;appareil et réglez en espèces ou Mobile Money.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2.5 pt-2">
                ...
                <div className="flex items-center justify-center gap-4 pt-3 text-[11px] text-slate-400">
                  <span className="flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> Colis Garanti 1 An</span>
                  <span className="flex items-center gap-1"><PackageCheck className="w-3.5 h-3.5 text-[#0047AB]" /> Paiement à la livraison</span>
                </div>
              </div>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4 max-w-lg mx-auto">
              ...
              <div className="text-center border-b border-slate-100 pb-4 mb-4">
                ...
                <div className="font-mono font-extrabold text-xl text-[#0047AB] tabular-nums">{fmt(selected.price)}</div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:outline-none focus:bg-white focus:border-[#0047AB]"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:outline-none focus:bg-white focus:border-[#0047AB] font-mono"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex : Akpakpa, rue pavée"
                    className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:outline-none focus:bg-white focus:border-[#0047AB]"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full mt-2 py-4 bg-[#0047AB] hover:bg-[#003580] text-white rounded-2xl font-bold text-sm sm:text-base shadow-lg shadow-blue-900/25 transition-all duration-150 active:scale-[0.98] flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60"
              >
                {submitting ? (
                  <>
                  ...
                  <>
                    <span>VALIDER MA COMMANDE ({fmt(selected.price)})</span>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/80 p-2.5 px-4 shadow-[0_-8px_20px_rgba(0,0,0,0.08)] flex items-center justify-between gap-2 max-w-full overflow-hidden">
        <div>
          ...
          <div className="font-display font-extrabold text-base text-[#0047AB] leading-none">
            {fmt(selected.price)}
          </div>
        </div>
        <button
          onClick={handleCtaClick}
          className="bg-[#0047AB] hover:bg-[#003580] text-white font-bold text-xs py-2.5 px-5 rounded-xl shadow-md transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
        >
          <span>Commander</span>
          <ArrowRight className="w-3.5 h-3.5" />
        </button>
      ...
      <a
        href={`https://wa.me/2290192901817?text=${encodeURIComponent(
          "Bonjour ! J'ai une question concernant l'éplucheur automatique ChefPeel™ Pro."
        )}`}
        target="_blank"
        rel="noopener noreferrer"
